...
class Figure {
    ...
    static isSamePoint(point1, point2) {
        return point1 && point2 && point1.x && point1.y && point2.x && point2.y &&
               point1.x === point2.x && point1.y === point2.y;
    }

    static isSameLine(line1, line2) { 
        let retVal = line1 && line2 && line1.points && line2.points &&
    ...
    cleanUp() {
        ...
        this.elements.points = this.elements.points.reduce(
            (p, c) => p.length === 0 || !Figure.isSamePoint(p[p.length - 1], c) ? p.concat(c) : p,
            []
        );

        this.sortLines();
        this.elements.lines = this.elements.lines.reduce(
            (p, c) => p.length === 0 || !Figure.isSameLine(p[p.length - 1], c) ? p.concat(c) : p,
            []
        );
    }
}
